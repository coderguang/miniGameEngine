...
int csg::CChatClient::doSend()
{
	do
	{
		...
		LogDebug("doSend Thread=" << ToStr(CThread::threadId()) << ",port="<<ToStr(_port)<<",length="<<ToStr(sResult)<<",say" << str);
...
bool csg::CChatClient::init()
{
	...
	{
		LogDebug("CChatClient::init failed,invalid socket");
		return false;
	}
...
void csg::CChatClient::doRecvChat()
{
	do
	{
		...
		{
			buffer[n - 1] = '\0';
			LogDebug("doRecvChat Thread=" << ToStr(CThread::threadId()) << ",say:" << std::string(buffer));
		}
