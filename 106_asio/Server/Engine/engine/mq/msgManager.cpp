...
bool csg::CMsgManager::regist(const IMsgBasePtr& msgBase)
{
	...
	if ( it != _msgMap.end() )
	{
		LogErr("CMsgManager::regist  duplicate type=" << ToStr(msgBase->getType()));
		return false;
	}
...
csg::IMsgBasePtr csg::CMsgManager::getMsg(int type)
{
	...
	LogErr("CMsgManager::getMsg no type=" << ToStr(type));
	return NULL;
}
...
csg::IMsgBasePtr csg::CMsgManager::createMsg(int type)
{
	...
	LogErr("CMsgManager::createMsg no type=" << ToStr(type));
	return NULL;
}
