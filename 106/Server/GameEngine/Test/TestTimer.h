...
{
	LogInfo("CTestTimer::handleTimeOut,flag="<<flag<<",name="<<_name<<",dt="<<current.asString());
	if ( _name == "3333" )
	{
		flag++;
		if ( 5 == flag )
		{
			...
			LogInfo("CTestTimer::handleTimeOut,cancel flag=" << flag << ",name=" << _name << ",dt=" << current.asString());
...
{
	LogInfo(" CTestTimer::handleException,flag=" << flag << ",name=" << _name << ",dt=" << CDateTime().asString() << ",ex.what=" << ex.what());;
}
