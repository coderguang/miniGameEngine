...
void testFirst()
{
	...
	//LogDebug("count1=" + ToStr(a->use_count()));
...
void testSp(){
	...
	LogDebug(t1->print());
	CTestSmartPtr t2 = t1;
	LogDebug("after t2"+t1->print());
	LogDebug("t2=" + t2->print());
	{
		CTestSmartPtr t3;
		try
		{
			LogDebug("t3=" + t3->print());
...
void testSmart3()
{
	...
	LogDebug("after t1" + t6->print());

	CTestSmartPtr t7 = CTestSmartPtr::dynamicCast(t6);

	LogDebug("after t7" + t7->print());
