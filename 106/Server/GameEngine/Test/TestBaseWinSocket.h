...
// 		cout << "threadId=" << CThread::threadId() << ",socket=" << PerHandleData->socketfd << "," << "A Client getSize: " << strlen(PerIoData->databuff.buf) <<" it say " std::endl;
		...
		LogDebug("socket=" << ToStr(PerHandleData->socketfd) << ", it say:"<<std::string(PerIoData->buffer));
...
int startClientC()
{
	...
	{
		LogDebug("winsocket socket failed,invalid socket");
	}

	struct sockaddr_in servaddr;
	servaddr.sin_family = AF_INET;
	servaddr.sin_port = htons(DefaultPort);
	servaddr.sin_addr.S_un.S_addr = inet_addr(localAddr_my);

	if ( SOCKET_ERROR == connect(clientfd ,( struct sockaddr * )&servaddr ,sizeof(servaddr)) )
	{
		LogDebug("winsocket socket failed,invalid socket""hello");
		return -1;
	}
