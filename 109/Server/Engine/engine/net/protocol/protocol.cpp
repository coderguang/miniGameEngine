...
csg::IProtocol::~IProtocol()
{
	
}

int csg::IProtocol::handleRecvData(const CSessionPtr session, const void* inData, const int len)
{
	CAutoLock lock(_readLock);
	...
	CCsgIoMgr::instance()->getLogicServer()->post(boost::bind(&IProtocol::handleReadData, this, session));
	return 0;
}

int csg::IProtocol::handleSendData(const CSessionPtr session, const void* data, const int len)
{
	CAutoLock l(_writeLock);
	_sendBuffer->append(data, len);
	session->getSocket()->async_write_some(boost::asio::null_buffers(), boost::bind(&IProtocol::handleWriteData, this, session, boost::asio::placeholders::error));
	return 0;
}

int csg::IProtocol::handleReadData(const CSessionPtr session)
{
...
int csg::IProtocol::pushMessage(const CSessionPtr session, const CMsgBlockPtr& mb)
{
...
int csg::IProtocol::handlePacket(const CSessionPtr session, const void *packageData, const int len)
{
...
int csg::IProtocol::handleWriteData(const CSessionPtr session, boost::system::error_code err)
{
	if (err)
	{
