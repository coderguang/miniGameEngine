...
public:
    ...
    static ConnectionString MONGO_CLIENT_FUNC
    parse(const std::string& address, std::string& errmsg);

    static ConnectionString MONGO_CLIENT_FUNC
    parseDeprecated(const std::string& address, std::string& errmsg);
...
public:
    ...
    */
    ...
    static std::string MONGO_CLIENT_FUNC
    createPasswordDigest(const std::string& username, const std::string& clearTextPassword);
    ...
    */
     ...
     * options.
     */
    bool createCollectionWithOptions(const std::string& ns,
                                     long long size = 0,
                                     bool capped = false,
                                     int max = 0,
    ...
    */
     ...
     */
    void group(const StringData& ns,
     ...
     */
    void groupWithKeyFunction(const StringData& ns,
                              const StringData& jsreduce,
                              std::vector<BSONObj>* output,
                              const BSONObj& initial = BSONObj(),
                              const Query& query = Query(),
...
private:
    ...
    void _buildGroupObj(const StringData& ns,
                        const StringData& jsreduce,
    ...
    void _runGroup(const StringData& ns,
                   const BSONObj& group,
                   const Query& query,
                   std::vector<BSONObj>* output);
...
public:
    ...
    const WriteConcern& getWriteConcern() const {
        return _writeConcern;
    }
    void setWriteConcern(const WriteConcern& w) {
        _writeConcern = w;
    }
    ...
    */
     ...
     */
    virtual void parallelScan(const StringData& ns,
                              int numCursors,
...
public:
    ...
    */
    ...
    static void MONGO_CLIENT_FUNC setLazyKillCursor(bool lazy) {
        _lazyKillCursor = lazy;
    }
    static bool MONGO_CLIENT_FUNC getLazyKillCursor() {
...
MONGO_CLIENT_API inline std::ostream& MONGO_CLIENT_FUNC
operator<<(std::ostream& s, const Query& q) {
    return s << q.toString();
}
